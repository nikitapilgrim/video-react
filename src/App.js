...
import videoDemo from './player/demo/demo.mp4'
...
const Demotest = ({video}) => {
    return <Standard video={video}/>
}

const DemoTwo = () => {
  const detectMobile = useMobileDetect();
  const isMobile = detectMobile.isMobile();
  const style = isMobile ? {} : { width: '80%', display: 'flex', margin: '0 auto' };
    function openInNewTab(url) {
        window.open(url, '_blank').focus();
    }
  const [active, setActive] = useState('http://commondatastorage.googleapis.com/gtv-videos-bucket/sample/WhatCarCanYouGetForAGrand.mp4');
    ...
    const handleChangeVideo = (video) => () => {
        openInNewTab(window.origin+'/video/'+video)
       // history.push(`/video/${video}`)
    }

  return (
      <div style={{...style}}>
          <ol style={{display: 'flex', flexDirection: 'column', cursor: 'pointer'}}>
              {videos.map((v, i) => <li onClick={handleChangeVideo(i)}><span>{v.title}</span></li>) }
          </ol>
